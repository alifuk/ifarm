...
$stmt = $conn->prepare("SELECT def.sekce, def.zobrazeni, def.nazev, def.volby, def.jednotky FROM `kategorie` INNER JOIN definicekategorie as def ON LCASE(kategorie.nazev) = LCASE(def.kategorie) WHERE kategorie.Id = ?   ");
...
$stmt->bind_result($sekce, $zobrazeni, $nazev, $volby, $jednotky);
$existuje = false;
$predchozisekce = "";
